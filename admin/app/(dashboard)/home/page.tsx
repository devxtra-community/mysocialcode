...
const ACTIVITY = [
  {
    color: '#16a34a',
    text: 'New user Aryan Mehta signed up via Google',
    time: '2 min ago',
  },
  {
    color: '#0891b2',
    text: "Event 'Neon Rave Night' went live — 340 tickets",
    time: '8 min ago',
  },
  {
    color: '#dc2626',
    text: 'User Kai Tanaka was banned by admin',
    time: '15 min ago',
  },
  {
    color: '#d97706',
    text: "Boost activated for 'Jazz Under Stars'",
    time: '22 min ago',
  },
  {
    color: '#16a34a',
    text: 'Revenue milestone: $48,000 crossed this month',
    time: '1 hr ago',
  },
  {
    color: '#7c3aed',
    text: "New event 'Tech Summit 2026' submitted for review",
    time: '2 hr ago',
  },
];
    ...
    <>
      ...
      <main className="p-7 flex flex-col gap-6 bg-gray-50 min-h-screen">
        ...
        <div className="bg-gradient-to-r from-cyan-500/10 to-violet-500/10 border border-cyan-200 rounded-2xl px-6 py-5 flex items-center justify-between">
          <div>
            <h2 className="font-display font-extrabold text-[20px] tracking-tight text-gray-900">
              Good morning, Admin 👋
            </h2>
        ...
        <div className="grid grid-cols-4 gap-4 lg:grid-cols-2">
          {STATS.map((s) => (
            <div
              key={s.label}
              className="relative bg-white border border-gray-200 rounded-2xl p-5 overflow-hidden hover:-translate-y-0.5 hover:shadow-md hover:border-cyan-300 transition-all duration-200 shadow-sm"
            >
              <div className="flex items-center justify-between mb-3">
                ...
                <span
                  className={`text-[11px] px-2 py-0.5 rounded-full font-medium ${s.up ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-500'}`}
                >
                  {s.up ? '▲' : '▼'} {s.change}
                </span>
              </div>
              <p className="font-display font-extrabold text-[26px] tracking-tight leading-none text-gray-900">
                {s.value}
              </p>
        ...
        <div className="grid grid-cols-[1fr_360px] gap-5 lg:grid-cols-1">
          ...
          <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm">
            <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
              <span className="font-display font-bold text-[14px] text-gray-900">
                Top Events This Month
              </span>
              <a
                href="/events"
                className="text-[11px] text-cyan-600 font-medium no-underline hover:opacity-70 transition-opacity"
              >
                VIEW ALL →
              </a>
            </div>
                ...
                <thead>
                  <tr className="border-b border-gray-100 bg-gray-50">
                    {['Event', 'Tickets', 'Revenue', 'Status'].map((h) => (
                      <th
                        key={h}
                        className="px-5 py-3 text-left text-[10px] tracking-[1.5px] uppercase text-gray-400 font-medium"
                      >
                        {h}
                      </th>
                    ))}
                  ...
                  {TOP_EVENTS.map((e) => (
                    ...
                    >
                      ...
                      <td className="px-5 py-3">
                        <span
                          className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] border font-medium ${statusCls[e.status]}`}
                        >
                          {e.status}
                        </span>
                      </td>
          ...
          <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm">
            ...
            <div>
              {ACTIVITY.map((a, i) => (
                <div
                  key={i}
                  className={`flex items-start gap-3 px-5 py-3 ${i < ACTIVITY.length - 1 ? 'border-b border-gray-100' : ''} hover:bg-gray-50 transition-colors`}
                >
                  <div
                    className="w-[7px] h-[7px] rounded-full mt-[6px] flex-shrink-0"
                    style={{
                      background: a.color,
